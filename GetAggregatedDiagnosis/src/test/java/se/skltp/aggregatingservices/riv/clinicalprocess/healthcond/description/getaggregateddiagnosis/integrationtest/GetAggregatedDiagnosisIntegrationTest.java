...
public class GetAggregatedDiagnosisIntegrationTest extends AbstractAggregateIntegrationTest {

    public GetAggregatedDiagnosisIntegrationTest() {
        super(rb.getString("TAK_TJANSTEKONTRAKT"));
    }

    @SuppressWarnings("unused")
    private static final Logger log = LoggerFactory.getLogger(GetAggregatedDiagnosisIntegrationTest.class);
    ...
    private static final String DEFAULT_SERVICE_ADDRESS = GetAggregatedDiagnosisMuleServer.getAddress("SERVICE_INBOUND_URL");

    protected String getConfigResources() {
        return "soitoolkit-mule-jms-connector-activemq-embedded.xml," + "GetAggregatedDiagnosis-common.xml," +
        // Only load GetAggregatedDiagnosis-common.xml, it will import the other config files since mule-deploy.properties can't load
        // config-files from jar-files on the classpath, for example agp-core.jar
    ...
    /**
     * Perform a test that is expected to return an exception due to missing mandatory http headers (sender-id, original-consumer-id, correlation-id)
     */
    @Test
     ...
     * @return
     */
    private List<ProcessingStatusRecordType> doTest(String              registeredResidentId, 
                                                    String              senderId, 
                                                    String              originalConsumerHsaId, 
                                                    String              correlationId,
                                                    int                 expectedProcessingStatusSize, 
                                                    TestData... testData) {

        // Setup and perform the call to the web service
        GetAggregatedDiagnosisTestConsumer consumer = new GetAggregatedDiagnosisTestConsumer(DEFAULT_SERVICE_ADDRESS, senderId, originalConsumerHsaId, correlationId);
        ...
        GetDiagnosisResponseType response = responseHolder.value;
        int expextedResponseSize = testData.length;
        assertEquals(expextedResponseSize, response.getDiagnosis().size());

        for (int i = 0; i < testData.length; i++) {
            DiagnosisType responseElement = response.getDiagnosis().get(i);
            assertEquals(registeredResidentId, responseElement.getDiagnosisHeader().getPatientId().getId());
            assertEquals(testData[i].getExpectedLogicalAddress(), responseElement.getDiagnosisHeader().getSourceSystemHSAId());
        }
        ...
        // Verify that correct "x-skltp-correlation-id" http header was passed to the engagement index
        assertEquals(SAMPLE_CORRELATION_ID, EngagemangsindexTestProducerLogger.getLastCorrelationId());

        // Verify that correct x-vp-sender-id, x-rivta-original-serviceconsumer-hsaid and x-skltp-correaltion-id http headers were passed to
